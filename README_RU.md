<p align="center">
  <img src="logo.png" width="650" alt="Datvex prompt LAB Logo">
</p>

<p align="center">
  <b>Выберите язык:</b><br>
  <a href="README.md"><img src="https://img.shields.io/badge/Английский-Current-brightgreen" alt="EN"></a>
  <a href="README_RU.md"><img src="https://img.shields.io/badge/Русский-Текущий-darkblue" alt="RU"></a>
  <a href="README_ZH.md"><img src="https://img.shields.io/badge/简体中文-yellow" alt="ZH"></a>
  <a href="README_ES.md"><img src="https://img.shields.io/badge/Español-orange" alt="ES"></a>
  <a href="README_HI.md"><img src="https://img.shields.io/badge/Hindi-teal" alt="HI"></a>
  <br>
  <a href="README_FR.md"><img src="https://img.shields.io/badge/Français-blue" alt="FR"></a>
  <a href="README_DE.md"><img src="https://img.shields.io/badge/Deutsch-darkgreen" alt="DE"></a>
  <a href="README_IT.md"><img src="https://img.shields.io/badge/Italiano-red" alt="IT"></a>
  <a href="README_PT.md"><img src="https://img.shields.io/badge/Português-purple" alt="PT"></a>
  <a href="README_JA.md"><img src="https://img.shields.io/badge/日本語-b7003a" alt="JA"></a>
  <a href="README_KO.md"><img src="https://img.shields.io/badge/한국어-green" alt="KO"></a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Промптов-12%2C033-blue" alt="Prompts Count">
</p>

---

# Datvex prompt LAB

Добро пожаловать в **Datvex prompt LAB**. Это профессиональный структурированный репозиторий, содержащий высококачественные промпты на английском языке, разработанные для специализированных задач в различных системах ИИ.

Коллекция включает в себя тщательно отобранные промпты, собранные из различных источников мирового сообщества ИИ — включая репозитории GitHub, каналы Discord и другие профессиональные ресурсы, — а также оригинальные промпты, разработанные специально для этой лаборатории.

---

## 📂 Навигация
Используйте библиотеку в зависимости от ваших задач. На данном этапе все ссылки ведут на официальную курируемую базу данных:

*   [**✨ ИИ поиск промптов (НОВИНКА)**](https://datvex.github.io/Datvex-prompt-LAB/) — Умный поиск на основе ИИ для быстрого нахождения идеальных промптов для ваших задач.

---

## 🚀 План развития (Roadmap)
 Мы постоянно развиваемся, чтобы предоставлять лучшие ресурсы для работы с нейросетями.

- [x] **Этап 1:** Запуск официальной курируемой базы данных (Текущий этап).
- [ ] **Этап 2:** Пользователи смогут добавлять свои промпты в базу данных.
- [ ] **Этап 3:** **Создание ИИ Генератора промптов.** Разработка интеллектуального инструмента для генерации промптов на основе ваших потребностей.

---

## 📜 Лицензия
Этот проект распространяется под лицензией **MIT**.
